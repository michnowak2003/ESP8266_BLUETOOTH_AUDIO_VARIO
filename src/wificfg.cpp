...
static String server_string_processor(const String& var){
    ...
    if(var == "ALARM_COLOR_1"){
        return String(Nvd.par.cfg.vario.alarmColor1);
        }
    else
...
void wificfg_ap_server_init() {
    ...
    pServer->on("/get", HTTP_GET, [] (AsyncWebServerRequest *request) {
		...
		if (request->hasParam("alarmColor1")) {
            inputMessage = request->getParam("alarmColor1")->value();
            bChange = true; 
            cfg.vario.alarmColor1 = inputMessage.toInt();
            }
